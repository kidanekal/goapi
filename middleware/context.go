...
var (
	...
	packageLogger = logger.CLI()
)

func init() {
	timeout, err := time.ParseDuration(os.Getenv("CONTEXT_TIMEOUT"))
	// timeout is a minimum of 5 seconds
	...
	// Fail fast on network layer (especially since calls are to loopback)
...
func Context(route string, inputLog log15.Logger, hdl Handle) httprouter.Handle {
	...
	return func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		ctx, cancel := context.WithTimeout(bgContext, ctxTimeout)
		defer cancel()
